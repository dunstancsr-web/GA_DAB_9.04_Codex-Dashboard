...
default_start = (
    (max_date - pd.DateOffset(months=12)).date()
...
col3.metric(
    "% Positive (4-5)",
    f"{(filtered['rating'].ge(4).mean() * 100):.1f}%" if len(filtered) else "—",
)
col4.metric(
    "Median length",
    f"{filtered['text_length'].median():.0f} chars" if len(filtered) else "—",
)
...
if len(filtered):
    ...
    summary_slot.markdown(
        "**Summary:** No reviews match the current filters."
    )
...
def render_wordcloud(text_series, stopwords, title):
    WordCloud, STOPWORDS, error = ensure_wordcloud()
    if error:
        st.error(
            "Unable to load the wordcloud library. "
            "Please install it with `pip install wordcloud` and restart the app."
        )
        return
...
base_stopwords = set()
WordCloud, STOPWORDS, _ = ensure_wordcloud()
if STOPWORDS:
